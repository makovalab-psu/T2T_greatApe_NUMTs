...
DIR='../../data/refs'
#DIR=/nfs/brubeck.bx.psu.edu/scratch4/edmundo/numts_t2t/data/primate_t2t_v2 #for BX server
GenomeDict2 = {
...
# Use tqdm to add a progress bar to the loop over rows
for row in tqdm(dataDF.index, desc="Processing rows", unit="row"):
    ...
    contig = str(dataDF.at[row, 'Loci'].split(":")[2])
    start = str(int(dataDF.at[row, 'Loci'].split(":")[3].split("-")[0]) - 500)
    end = str(int(dataDF.at[row, 'Loci'].split(":")[3].split("-")[1]) + 500)
    ...
    for match in ast.literal_eval(str(dataDF.at[row, 'Matches'])):
        querySpecies = str(dataDF.at[row, 'DB_Species'])
...
#dataDF.to_csv("matches_similarity_results.csv", index=None)
dataDF.to_csv(f"matches_similarity_results.flanks{FLANK_SIZE}.tab", index=None, sep='\t')


def expand_columns(file_path=f'matches_similarity_results.tab',
                   output_file_path=f'expanded_matches_similarity_results.tab'):
    """
